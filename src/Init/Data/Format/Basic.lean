...
- `canFlatten` means that the group is allowed to be, and has space to be, flattened.
- `cannotFlatten` means that the group could have been flattened, but there is insufficient space.
- `mustNotFlatten` means that this group should not be flattened irrespective of space concerns.
...
/--
Given that flattening is permissible, returns a flatten directive that allows flattening if the
argument is `true` or disallows it if the argument is `false`.
-/
def FlattenDirective.isPossible : Bool → FlattenDirective
  | true => canFlatten
  | false => cannotFlatten
...
private def pushGroup (flb : FlattenBehavior) (items : List WorkItem) (gs : List WorkGroup) (w : Nat) [Monad m] [MonadPrettyFormat m] : m (List WorkGroup) := do
  ...
  let g  := { fld := .isPossible (flb == FlattenBehavior.allOrNone), flb := flb, items := items : WorkGroup }
  ...
  return { g with fld := .isPossible (!r.foundFlattenedHardLine && r'.space <= w-k) }::gs
...
def prettyM (f : Format) (w : Nat) (indent : Nat := 0) [Monad m] [MonadPrettyFormat m] : m Unit :=
  be w [{ flb := FlattenBehavior.allOrNone, fld := .mustNotFlatten, items := [{ f := f, indent, activeTags := 0 }]}]
