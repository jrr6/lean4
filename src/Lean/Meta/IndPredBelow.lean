...
def mkBelow (declName : Name) : MetaM Unit := do
  if (← isInductivePredicate declName) then
    ...
    if x.isRec && !x.isNested then
      ...
      ctx.belowNames.forM (ensureAuxNameUnused · declName)
      addDecl decl
      ...
      for i in [:ctx.typeInfos.size] do
        let decl ← try
          ...
          trace[Meta.IndPredBelow] "failed to prove brecOn for {ctx.belowNames[i]!}\n{e.toMessageData}"
          continue
        ensureAuxNameUnused decl.getNames[0]! declName
        try
          -- disable async TC so we can catch its exceptions
          withOptions (Elab.async.set · false) do
            addDecl decl
        catch e =>
          trace[Meta.IndPredBelow] "failed to add brecOn decl for {ctx.belowNames[i]!}\n{e.toMessageData}"
